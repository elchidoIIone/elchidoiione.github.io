<?php
session_start();
if (!isset($_SESSION["user_login"])) { header("Location: index.html"); exit; }
...
$gradesQuery = $conn->query("SELECT g.studentsID, g.activityID, g.grade FROM grades g JOIN activities a ON g.activityID = a.activityID WHERE a.subjectID = $currentSubjectID");
$grades = [];
while ($g = $gradesQuery->fetch_assoc()) { $grades[$g['studentsID']][$g['activityID']] = $g['grade']; }
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    <section>
        ...
        <div id="mnBody">
            <div id="selectSection"> <?php include "./layouts/icons.php" ?> </div>
            <div id="table">
                ...
                <div id="schlInputs">
                    <form action="./php/saveActivity.php" method="POST">
                        <fieldset>
                            ...
                            <!-- CORRECCIÓN: REDIRECT A ENGLISH.PHP -->
                            <input type="hidden" name="redirect" value="../english.php">
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <?php foreach ($activities as $act): ?>
                                    <th><?= htmlspecialchars($act['activityName']) ?><br><span style="font-size: 0.8em; opacity: 0.8; font-weight: normal;">(ID: <?= $act['activityID'] ?>)</span></th>
                                <?php endforeach; ?>
                                <th>Promedio</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($students as $stu): ?>
                                <?php $suma = 0; $cantidad = 0; ?>
                                <tr class="student-row">
                                    <td style="text-align: left; font-weight: bold; padding-left: 15px;"><?= htmlspecialchars($stu['name']) ?></td>
                                    <?php foreach ($activities as $act): ?>
                                        <?php 
                                            $nota = $grades[$stu['studentsID']][$act['activityID']] ?? null;
                                            if ($nota !== null && $nota !== "") { $suma += floatval($nota); $cantidad++; }
                                        ?>
                                        <td class="editable" data-student="<?= $stu['studentsID'] ?>" data-activity="<?= $act['activityID'] ?>"><?= $nota ?? "-" ?></td>
                                    <?php endforeach; ?>
                                    <td class="average-cell" style="font-weight: bold;"><?php echo ($cantidad > 0) ? number_format($suma / $cantidad, 1) : "-"; ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
    ...
    document.querySelectorAll(".editable").forEach(cell => {
        cell.addEventListener("click", function () {
            ...
            input.type = "number"; input.min = 0; input.max = 100; input.value = currentVal;
            input.style.width = "100%"; input.style.height = "100%"; input.style.textAlign = "center";
            input.style.border = "none"; input.style.outline = "2px solid #800020"; input.style.background = "#fff";
            input.style.fontSize = "inherit"; input.style.fontWeight = "bold";
            this.innerHTML = ""; this.appendChild(input); input.focus();
            input.addEventListener("blur", () => {
                let val = input.value;
                if (val !== "") { let num = parseFloat(val); if (num < 0) val = "0"; if (num > 100) val = "100"; }
                saveData(this, val);
            });
    ...
    function saveData(cell, value) {
        ...
        let studentID = cell.dataset.student; let activityID = cell.dataset.activity;
    ...
    function updateRowAverage(row) {
        ...
        row.querySelectorAll(".editable").forEach(c => {
            let val = parseFloat(c.innerText); if (!isNaN(val)) { sum += val; count++; }
        });
        let avgCell = row.querySelector(".average-cell");
        avgCell.innerText = (count > 0) ? (sum / count).toFixed(1) : "-";
    }
    </script>
</body>
</html>